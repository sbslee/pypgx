...
* :issue:`176`: Fix bug in :meth:`api.core.get_recommendation` method where a single-gene lookup on a two-gene drug returned the recommendation for an arbitrary phenotype of the unspecified gene instead of the one for that gene being ungenotyped.
